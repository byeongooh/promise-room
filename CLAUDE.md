...
**바로 다음 — 개인 달력 (2026-08-21 결정)**
"캘박"(캘린더에 박아둔다) 문화를 앱 안으로 가져온다. 한 명이 플랜을 만들면
참여자 **모두의 달력에 자동으로 들어가고**, 달력에서 그 항목을 누르면 플랜
상세로 간다. 개인 메모도 같은 달력에 남긴다.
...
새로 필요한 것:
- 월 그리드 화면 + 탭바에 항목 추가(지금 3개 → 4개)
- 개인 메모 저장처. **`users/{uid}` 문서가 수확에도 필요하므로 같이 만든다** —
  그러면 보안 규칙을 한 번만 배포하면 된다(사람이 직접 해야 하는 일).
- 메모는 하위 컬렉션(`users/{uid}/notes/{id}`)이 맞다. 배열로 두면 계속 쌓여
  문서가 무거워진다 — `placeSuggestions`와 판단이 다른 이유가 이것이다.
