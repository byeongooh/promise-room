...
import { addNote, listNotes, removeNote, setNoteDone } from "@/lib/note-service";
...
const addSchema = z.object({
  ...
  /** 이 약속에 딸린 메모면 그 id. 없으면 그날에만 딸린 메모다. */
  promiseId: z.string().max(120).nullish(),
});
...
export const POST = withCaller(async (caller, req) => {
  ...
  const note = await addNote(caller, parsed.data.date, parsed.data.text, parsed.data.promiseId);
...
const doneSchema = z.object({ id: z.string().min(1), done: z.boolean() });

/** 챙겼는지 표시. 체크리스트로 쓰일 때만 쓴다. */
export const PATCH = withCaller(async (caller, req) => {
  const parsed = doneSchema.safeParse(await req.json().catch(() => null));
  if (!parsed.success) throw badRequest("입력값이 올바르지 않습니다.");
  await setNoteDone(caller, parsed.data.id, parsed.data.done);
  return NextResponse.json({ ok: true });
});
